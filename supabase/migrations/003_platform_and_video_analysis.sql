-- 003 platform layer + tiedown_runs for tiedown-mobile-app
...
-- Discipline run log for tie-down roping.
create table if not exists public.tiedown_runs (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users (id) on delete cascade,
  total_time numeric,
  catch_type text,
  barrier_broken boolean not null default false,
  tie_held boolean not null default false,
  notes text,
  created_at timestamptz not null default now()
);

create index if not exists tiedown_runs_user_idx on public.tiedown_runs (user_id, created_at desc);

alter table public.tiedown_runs enable row level security;

drop policy if exists "tiedown_runs_owner_all" on public.tiedown_runs;
create policy "tiedown_runs_owner_all" on public.tiedown_runs
