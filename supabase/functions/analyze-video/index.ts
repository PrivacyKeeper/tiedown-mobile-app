...
// analyze-video edge function for tie-down roping.
//
...
const ANALYSIS_SCHEMA = {
  ...
  properties: {
    ...
    phases: {
      type: "object",
      additionalProperties: false,
      required: ["rope_delivery", "dismount", "tie", "footwork"],
      properties: {
          rope_delivery: { type: "string" },
          dismount: { type: "string" },
          tie: { type: "string" },
...
const SYSTEM_PROMPT =
  "You are an expert tie-down roping coach. Analyze the athlete's run from the supplied frames. " +
  ...
  try {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: SYSTEM_PROMPT + " JSON schema: " + JSON.stringify(ANALYSIS_SCHEMA) },
          {
            role: "user",
            content: [
              { type: "text", text: "Analyze this tie-down roping run." },
              ...images,
            ],
          },
        ],
    ...
    if (supabaseUrl && serviceKey && user_id) {
      ...
      await admin.from("video_analysis_results").insert({
        user_id,
        event_type: body.event_type ?? "tiedown",
        video_url,
